  ...
  <template>
    <div style="padding: 10px">
      ...
      <el-table :data="tableData" border stripe style="width: 100%">
        ...
        <el-table-column prop="receiver" label="收件人" width="200"/>
        ...
        <el-table-column fixed="right" label="操作" width="200">
          <template #default="scope">
            ...
            <el-popconfirm title="确认发货？" @confirm="handleShip(scope.row)">
              <template #reference>
                <el-button type="warning" :disabled="shipable(scope.row.cargoSituation)">发货</el-button>
              </template>
            </el-popconfirm>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    load(){
      request.get("/transport/shipment/" + JSON.parse(sessionStorage.getItem("user")).id
      ).then(res=>{
        console.log(res);
    ...
    handleShip(row){
      request.put("/transport/ship", row).then(res =>{
        if(res.code === '0'){
          ElMessage({
            type:"success",
            message:"发货成功"
